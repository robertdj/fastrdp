...
The example above is executed with *fastrdp* in about a millisecond on my machine 
...
"Structured" means that the output of `rdp` is **much** smaller than the input.
The figure is produced with the script in the `performance` folder.
...
```bash
clang++ -std=c++20 -Ofast measure_performance.cpp -o measure_performance
```
